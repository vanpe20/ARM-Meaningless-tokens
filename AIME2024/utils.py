...
def choose_model(model, arm = True):
    if arm is True:
        if 'Qwen2.5' in model:
            model = Qwen2ForCausalLM.from_pretrained(model).to(args.device)
        elif 'Llama' in model:
            model = LlamaForCausalLM.from_pretrained(model).to(args.device)
        elif 'gemma' in model:
            model = Gemma3ForCausalLM.from_pretrained(model).to(args.device)
    else:
        model = AutoModelForCausalLM.from_pretrained(model, trust_remote_code=True).to(args.device)
